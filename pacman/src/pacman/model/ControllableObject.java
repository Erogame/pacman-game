...
public abstract class ControllableObject extends Sprite {
	...
	public void move() {
		...
		if (_direction == Direction.ARRIBA) {
			if (getCollisionMap().puedeMoverse(this, getPosition().x, getPosition().y - 1))
				dy -= _velocity;
			else
				setDirection(Direction.NONE);
			
		} else if (_direction == Direction.ABAJO) {
			if (getCollisionMap().puedeMoverse(this, getPosition().x, getPosition().y + 1))
				dy += _velocity;
			else
				setDirection(Direction.NONE);
			
		} else if (_direction == Direction.DERECHA) {
			if (getCollisionMap().puedeMoverse(this, getPosition().x + 1, getPosition().y))
				dx += _velocity;
			else
				setDirection(Direction.NONE);
			
		} else if (_direction == Direction.IZQUIERDA) {
			if (getCollisionMap().puedeMoverse(this, getPosition().x - 1, getPosition().y))
				dx -= _velocity;
			else
				setDirection(Direction.NONE);
	...
	public void setPosition(int x, int y) {
		if (_gameMap.puedeMoverse(this, x, y)) {
			_position.move(x, y);
		}
